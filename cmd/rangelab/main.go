...
func runWithArgs(args []string) error {
	...
	if *futuresBTCRegimeETHSOLContextAudit {
		if sourceManifest.ComparisonOnly || sourceManifest.Product != "Binance USDT-M futures" {
			return fmt.Errorf("-futures-btc-regime-eth-sol-context-audit requires Binance USDT-M futures source; got product=%q comparison_only=%t", sourceManifest.Product, sourceManifest.ComparisonOnly)
		}
		if tradeProducingFlagSelected {
			return fmt.Errorf("-futures-btc-regime-eth-sol-context-audit cannot be combined with trade-producing prototype/backtest/optimization/replay/walk-forward flags")
		}
	...
	if *futuresDerivativesContextAudit {
		if sourceManifest.ComparisonOnly || sourceManifest.Product != "Binance USDT-M futures" {
			return fmt.Errorf("-futures-derivatives-context-audit requires Binance USDT-M futures source; got product=%q comparison_only=%t", sourceManifest.Product, sourceManifest.ComparisonOnly)
		}
		if tradeProducingFlagSelected {
			return fmt.Errorf("-futures-derivatives-context-audit cannot be combined with trade-producing prototype/backtest/optimization/replay/walk-forward flags")
		}
		if *detector || *detectorSweep || *detectorDurabilitySweep || *detectorContextRefinementAudit || *holdInsideDirectionalEdgeAudit || *holdInsideMidlineTransitionAudit || *holdInsideMidlineReactionAudit || *futuresImpulseAbsorptionAudit || *futuresRangeCandidateDiscoveryAudit || *futuresRangeUniverseDiscoveryAudit || *futuresHigherTFNestedRangeRotationAudit || *futuresRangeContextTriageAudit || *futuresRangeStateConstructionLoopAudit || *futuresRangeContextRouterAudit || *futuresRangeRouterRotationPremiseAudit || *futuresBTCRegimeETHSOLContextAudit || *futuresDerivativesContextSourceAudit || *srAudit || *srBoundaryAudit || *srBoundaryInspect || *srRejectionTimingAudit || *srConfirmationTimingAudit || *srFalseBreakReclaimTimingAudit || *compressionBreakoutAudit || *rangeRegimeDurabilityAudit {
	...
	if *futuresBTCRegimeETHSOLContextAudit {
		contextCfg := futuresBTCRegimeETHSOLContextAuditConfigForRun()
		var err error
		btcRegimeETHSOLContextResult, err = lab.RunFuturesBTCRegimeETHSOLContextAudit(contextCfg, lab.DefaultSplits())
		if err != nil {
			return err
		}
		if err := writeJSON(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_sources.json"), btcRegimeETHSOLContextResult.SourceRows); err != nil {
			return err
		}
		if err := writeFuturesBTCRegimeETHSOLContextSourcesCSV(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_sources.csv"), btcRegimeETHSOLContextResult.SourceRows); err != nil {
			return err
		}
		if err := writeJSON(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_coverage.json"), btcRegimeETHSOLContextResult.CoverageRows); err != nil {
			return err
		}
		if err := writeFuturesBTCRegimeETHSOLContextCoverageCSV(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_coverage.csv"), btcRegimeETHSOLContextResult.CoverageRows); err != nil {
			return err
		}
		if err := writeJSON(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_btc_states.json"), btcRegimeETHSOLContextResult.BTCStateRows); err != nil {
			return err
		}
		if err := writeFuturesBTCRegimeETHSOLContextBTCStatesCSV(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_btc_states.csv"), btcRegimeETHSOLContextResult.BTCStateRows); err != nil {
			return err
		}
		if err := writeJSON(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_local_states.json"), btcRegimeETHSOLContextResult.LocalStateRows); err != nil {
			return err
		}
		if err := writeFuturesBTCRegimeETHSOLContextLocalStatesCSV(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_local_states.csv"), btcRegimeETHSOLContextResult.LocalStateRows); err != nil {
			return err
		}
		if err := writeJSON(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_relative_strength.json"), btcRegimeETHSOLContextResult.RelativeStrengthRows); err != nil {
			return err
		}
		if err := writeFuturesBTCRegimeETHSOLContextRelativeStrengthCSV(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_relative_strength.csv"), btcRegimeETHSOLContextResult.RelativeStrengthRows); err != nil {
			return err
		}
		if err := writeJSON(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_labels.json"), btcRegimeETHSOLContextResult.LabelRows); err != nil {
			return err
		}
		if err := writeFuturesBTCRegimeETHSOLContextLabelsCSV(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_labels.csv"), btcRegimeETHSOLContextResult.LabelRows); err != nil {
			return err
		}
		if err := writeJSON(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_cohorts.json"), btcRegimeETHSOLContextResult.CohortRows); err != nil {
			return err
		}
		if err := writeFuturesBTCRegimeETHSOLContextCohortsCSV(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_cohorts.csv"), btcRegimeETHSOLContextResult.CohortRows); err != nil {
			return err
		}
		if err := writeJSON(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_rankings.json"), btcRegimeETHSOLContextResult.RankingRows); err != nil {
			return err
		}
		if err := writeFuturesBTCRegimeETHSOLContextRankingsCSV(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_rankings.csv"), btcRegimeETHSOLContextResult.RankingRows); err != nil {
			return err
		}
		if err := writeJSON(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_summary.json"), btcRegimeETHSOLContextResult.SummaryRows); err != nil {
			return err
		}
		if err := writeFuturesBTCRegimeETHSOLContextSummaryCSV(filepath.Join(*outDir, "futures_btc_regime_eth_sol_context_summary.csv"), btcRegimeETHSOLContextResult.SummaryRows); err != nil {
			return err
		}
		fmt.Printf("futures_btc_regime_eth_sol_context_audit source_rows=%d coverage_rows=%d btc_state_rows=%d local_state_rows=%d relative_strength_rows=%d label_rows=%d cohort_rows=%d ranking_rows=%d passing_cohorts=%d stop_state=%s\n",
			len(btcRegimeETHSOLContextResult.SourceRows),
			len(btcRegimeETHSOLContextResult.CoverageRows),
			len(btcRegimeETHSOLContextResult.BTCStateRows),
			len(btcRegimeETHSOLContextResult.LocalStateRows),
			len(btcRegimeETHSOLContextResult.RelativeStrengthRows),
			len(btcRegimeETHSOLContextResult.LabelRows),
			len(btcRegimeETHSOLContextResult.CohortRows),
	...
	if *futuresDerivativesContextAudit {
		derivCfg := futuresDerivativesContextAuditConfigForRun()
		var err error
		derivativesContextAuditResult, err = lab.RunFuturesDerivativesContextAudit(derivCfg, lab.DefaultSplits())
		if err != nil {
			return err
		}
		artifacts := []struct {
			...
			{"futures_derivatives_context_sources", derivativesContextAuditResult.SourceRows},
			{"futures_derivatives_context_coverage", derivativesContextAuditResult.CoverageRows},
			{"futures_derivatives_context_basis_features", derivativesContextAuditResult.BasisFeatureRows},
			{"futures_derivatives_context_local_states", derivativesContextAuditResult.LocalStateRows},
			{"futures_derivatives_context_labels", derivativesContextAuditResult.LabelRows},
			{"futures_derivatives_context_cohorts", derivativesContextAuditResult.CohortRows},
			{"futures_derivatives_context_rankings", derivativesContextAuditResult.RankingRows},
			{"futures_derivatives_context_missingness", derivativesContextAuditResult.MissingnessRows},
			{"futures_derivatives_context_summary", derivativesContextAuditResult.SummaryRows},
		}
		for _, art := range artifacts {
			if err := writeJSON(filepath.Join(*outDir, art.name+".json"), art.rows); err != nil {
				return err
		...
		if err := writeFuturesDerivativesContextAuditSourcesCSV(filepath.Join(*outDir, "futures_derivatives_context_sources.csv"), derivativesContextAuditResult.SourceRows); err != nil {
			return err
		}
		if err := writeFuturesDerivativesContextAuditCoverageCSV(filepath.Join(*outDir, "futures_derivatives_context_coverage.csv"), derivativesContextAuditResult.CoverageRows); err != nil {
			return err
		}
		if err := writeFuturesDerivativesContextAuditBasisFeaturesCSV(filepath.Join(*outDir, "futures_derivatives_context_basis_features.csv"), derivativesContextAuditResult.BasisFeatureRows); err != nil {
			return err
		}
		if err := writeFuturesDerivativesContextAuditLocalStatesCSV(filepath.Join(*outDir, "futures_derivatives_context_local_states.csv"), derivativesContextAuditResult.LocalStateRows); err != nil {
			return err
		}
		if err := writeFuturesDerivativesContextAuditLabelsCSV(filepath.Join(*outDir, "futures_derivatives_context_labels.csv"), derivativesContextAuditResult.LabelRows); err != nil {
			return err
		}
		if err := writeFuturesDerivativesContextAuditCohortsCSV(filepath.Join(*outDir, "futures_derivatives_context_cohorts.csv"), derivativesContextAuditResult.CohortRows); err != nil {
			return err
		}
		if err := writeFuturesDerivativesContextAuditRankingsCSV(filepath.Join(*outDir, "futures_derivatives_context_rankings.csv"), derivativesContextAuditResult.RankingRows); err != nil {
			return err
		}
		if err := writeFuturesDerivativesContextAuditMissingnessCSV(filepath.Join(*outDir, "futures_derivatives_context_missingness.csv"), derivativesContextAuditResult.MissingnessRows); err != nil {
			return err
		}
		if err := writeFuturesDerivativesContextAuditSummaryCSV(filepath.Join(*outDir, "futures_derivatives_context_summary.csv"), derivativesContextAuditResult.SummaryRows); err != nil {
			return err
		}
		fmt.Printf("futures_derivatives_context_audit source_rows=%d coverage_rows=%d basis_feature_rows=%d local_state_rows=%d label_rows=%d cohort_rows=%d ranking_rows=%d missingness_rows=%d passing_cohorts=%d stop_state=%s\n",
			len(derivativesContextAuditResult.SourceRows),
			len(derivativesContextAuditResult.CoverageRows),
			len(derivativesContextAuditResult.BasisFeatureRows),
			len(derivativesContextAuditResult.LocalStateRows),
			len(derivativesContextAuditResult.LabelRows),
			len(derivativesContextAuditResult.CohortRows),
			len(derivativesContextAuditResult.RankingRows),
...
func writeFuturesBTCRegimeETHSOLContextSourcesCSV(path string, rows []lab.FuturesBTCRegimeETHSOLContextSourceRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesBTCRegimeETHSOLContextCoverageCSV(path string, rows []lab.FuturesBTCRegimeETHSOLContextCoverageRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesBTCRegimeETHSOLContextBTCStatesCSV(path string, rows []lab.FuturesBTCRegimeETHSOLContextBTCStateRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesBTCRegimeETHSOLContextLocalStatesCSV(path string, rows []lab.FuturesBTCRegimeETHSOLContextLocalStateRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesBTCRegimeETHSOLContextRelativeStrengthCSV(path string, rows []lab.FuturesBTCRegimeETHSOLContextRelativeStrengthRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesBTCRegimeETHSOLContextLabelsCSV(path string, rows []lab.FuturesBTCRegimeETHSOLContextLabelRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesBTCRegimeETHSOLContextCohortsCSV(path string, rows []lab.FuturesBTCRegimeETHSOLContextCohortRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesBTCRegimeETHSOLContextRankingsCSV(path string, rows []lab.FuturesBTCRegimeETHSOLContextRankingRow) error {
	return writeJSONTaggedCSV(path, rows)
}
...
func writeFuturesDerivativesContextAuditSourcesCSV(path string, rows []lab.FuturesDerivativesContextAuditSourceRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesDerivativesContextAuditCoverageCSV(path string, rows []lab.FuturesDerivativesContextAuditCoverageRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesDerivativesContextAuditBasisFeaturesCSV(path string, rows []lab.FuturesDerivativesContextAuditBasisFeatureRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesDerivativesContextAuditLocalStatesCSV(path string, rows []lab.FuturesDerivativesContextAuditLocalStateRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesDerivativesContextAuditLabelsCSV(path string, rows []lab.FuturesDerivativesContextAuditLabelRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesDerivativesContextAuditCohortsCSV(path string, rows []lab.FuturesDerivativesContextAuditCohortRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesDerivativesContextAuditRankingsCSV(path string, rows []lab.FuturesDerivativesContextAuditRankingRow) error {
	return writeJSONTaggedCSV(path, rows)
}

func writeFuturesDerivativesContextAuditMissingnessCSV(path string, rows []lab.FuturesDerivativesContextAuditMissingnessRow) error {
	return writeJSONTaggedCSV(path, rows)
}
